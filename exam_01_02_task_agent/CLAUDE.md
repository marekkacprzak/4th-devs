...
2. For each suspect, queries `POST https://<hubapi>/api/location` to get their GPS coordinates
...
5. Queries `POST https://<hubapi>/api/accesslevel` with the suspect's name, surname, and birthYear (integer)
...
Follow the same structure as `../exam_01_01_task_agent/PeopleAgent/`:
...
- The suspect list comes from the exam_01_01_task_agent results (people from CSV who matched the "transport" criteria)
