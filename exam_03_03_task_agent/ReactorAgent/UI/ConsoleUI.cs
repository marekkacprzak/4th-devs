...
public static class ConsoleUI
{
    ...
    public static void PrintBoard(string boardText)
    {
        AnsiConsole.WriteLine();
        AnsiConsole.Write(new Panel(Markup.Escape(boardText))
            .Header("[bold cyan]Board State[/]")
            .BorderColor(Color.Cyan1)
            .RoundedBorder()
            .Padding(1, 0));
    }

    public static void PrintReactorBoard(ReactorBoard board)
    {
        var grid = board.ToTextGrid();
        AnsiConsole.WriteLine();
        AnsiConsole.Write(new Panel(Markup.Escape(grid))
            .Header($"[bold cyan]Reactor Board — Robot at col {board.RobotColumn}[/]")
            .BorderColor(Color.Cyan1)
            .RoundedBorder()
            .Padding(1, 0));
    }
