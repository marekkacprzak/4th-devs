...
{
    private static readonly ActivitySource Activity = new("Filesystem.Centrala");
    ...
    /// <summary>Batch mode: sends answer as an array of operations.</summary>
    public async Task<string> VerifyBatchAsync(object[] answers)
    {
        using var span = Activity.StartActivity("centrala.verify.batch");
        var url = $"{_config.ApiUrl}/verify";
        ConsoleUI.PrintStep($"POST {url} [batch: {answers.Length} operations]");
        var result = await PostJsonAsync(url, new
        {
            apikey = _config.ApiKey,
            task = _config.TaskName,
    ...
    /// <summary>Download raw bytes from a URL.</summary>
    public async Task<byte[]> GetBytesAsync(string url)
    {
        ConsoleUI.PrintStep($"GET {url}");
