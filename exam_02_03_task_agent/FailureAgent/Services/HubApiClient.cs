...
{
    private static readonly ActivitySource Activity = new("FailureAgent.Hub");
    ...
    public async Task<string> SubmitLogsAsync(string logs)
    {
        using var span = Activity.StartActivity("hub.submit_logs");
        span?.SetTag("logs.length", logs.Length);
