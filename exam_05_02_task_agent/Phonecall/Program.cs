...
using Phonecall.Adapters;
using Phonecall.Config;
using Phonecall.Services;
using Phonecall.Telemetry;
using Phonecall.UI;

// ── Load .env ────────────────────────────────────────────────────────────────
// Search up the directory tree so the agent works from both the project dir
// and the solution root (dotnet run --project Phonecall from repo root).
...
{
    Console.Error.WriteLine("[WARN] .env file not found. Hub__ApiKey and GEMINI_API_KEY will be empty.");
}
...
var audioService = new GeminiAudioService(httpClient, audioConfig, runLogger);
...
// ── Banner ────────────────────────────────────────────────────────────────────
ConsoleUI.PrintBanner("Phonecall", "Phonecall Agent — Tymon Gajewski");
ConsoleUI.PrintInfo($"LLM: {agentConfig.Provider} / {agentConfig.Model}");
ConsoleUI.PrintInfo($"TTS model: {audioConfig.TtsModel} (voice: {audioConfig.TtsVoice})");
ConsoleUI.PrintInfo($"STT model: {audioConfig.SttModel}");
...
if (string.IsNullOrWhiteSpace(audioConfig.GetApiKey()))
    errors.Add("GEMINI_API_KEY is empty. Set it in .env: GEMINI_API_KEY=<your-gemini-key>");
