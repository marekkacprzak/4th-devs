...
public static class ConsoleUI
{
    ...
    public static void PrintConversation(string speaker, string text)
    {
        var color = speaker == "Tymon" ? "cyan" : "yellow";
        AnsiConsole.Write(new Panel($"[{color}]{Markup.Escape(text)}[/]")
            .Header($"[{color}]{Markup.Escape(speaker)}[/]")
            .BorderColor(speaker == "Tymon" ? Color.Cyan1 : Color.Yellow)
            .RoundedBorder()
            .Padding(1, 0));
    }
