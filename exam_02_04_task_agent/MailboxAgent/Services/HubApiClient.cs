...
{
    private static readonly ActivitySource Activity = new("MailboxAgent.Hub");
    ...
    public async Task<string> SubmitAnswerAsync(string password, string date, string confirmationCode)
    {
        using var span = Activity.StartActivity("hub.submit_answer");
        span?.SetTag("answer.date", date);
        span?.SetTag("answer.has_password", !string.IsNullOrEmpty(password));
        span?.SetTag("answer.has_code", !string.IsNullOrEmpty(confirmationCode));
