...
namespace RadioMonitoring.Adapters;
...
public static class OpenAiClientFactory
{
    ...
    public static IChatClient CreateVisionChatClient(VisionConfig config, TelemetryConfig? telemetryConfig = null)
    {
        var client = CreateOpenAiClientFromVision(config);
    ...
    private static OpenAIClient CreateOpenAiClientFromVision(VisionConfig config)
    {
        return config.Provider.ToLowerInvariant() switch
        {
            "openai" => new OpenAIClient(config.GetApiKey()),
            "lmstudio" => new OpenAIClient(
                ...
                new OpenAIClientOptions
                {
                    ...
                    NetworkTimeout = TimeSpan.FromSeconds(300) // 5 min for large images
                }),
            _ => throw new InvalidOperationException(
                $"Unknown provider '{config.Provider}'. Supported: openai, lmstudio")
        };
