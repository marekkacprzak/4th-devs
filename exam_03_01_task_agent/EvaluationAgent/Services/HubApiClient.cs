...
{
    private static readonly ActivitySource Activity = new("EvaluationAgent.Hub");
    ...
    public async Task<string> SubmitEvaluationAsync(List<string> recheckIds)
    {
        using var span = Activity.StartActivity("hub.submit_evaluation");
        span?.SetTag("recheck.count", recheckIds.Count);

        var body = new
        {
            apikey = _config.ApiKey,
            task = _config.TaskName,
            answer = new { recheck = recheckIds }
        };
        ...
        for (int attempt = 1; attempt <= _config.MaxRetries; attempt++)
        {
            ...
            FileLogger.Log($"HUB REQUEST attempt={attempt}", json);
            ...
            FileLogger.Log($"HUB RESPONSE status={(int)response.StatusCode}", responseBody);
