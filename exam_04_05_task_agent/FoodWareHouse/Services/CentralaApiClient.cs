...
{
    private static readonly ActivitySource Activity = new("FoodWareHouse.Centrala");
    ...
    /// <summary>Download text from a URL.</summary>
    public async Task<string> GetStringAsync(string url)
    {
        ConsoleUI.PrintStep($"GET {url}");
        _logger.LogInfo($"GET {url}");
        var body = await _http.GetStringAsync(url);
        _logger.LogInfo($"GET response ({body.Length} chars): {body[..Math.Min(500, body.Length)]}");
        return body;
    }
