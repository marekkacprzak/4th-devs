...
public static class ConsoleUI
{
    ...
    public static void PrintPhase(int phase, string name)
    {
        AnsiConsole.WriteLine();
        AnsiConsole.Write(new Rule($"[bold magenta]══ PHASE {phase}: {Markup.Escape(name)} ══[/]")
            .RuleStyle("magenta"));
        AnsiConsole.WriteLine();
    }

    public static void PrintTimer(int secondsLeft)
    {
        var color = secondsLeft > 20 ? "green" : secondsLeft > 10 ? "yellow" : "red";
        AnsiConsole.MarkupLine($"  ⏱  [bold {color}]{secondsLeft}s remaining[/]");
    }
