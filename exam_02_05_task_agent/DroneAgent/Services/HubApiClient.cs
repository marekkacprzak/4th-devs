...
namespace DroneAgent.Services;

public class HubApiClient
{
    private static readonly ActivitySource Activity = new("DroneAgent.Hub");
    ...
    public async Task<string> SubmitInstructionsAsync(string[] instructions)
    {
        using var span = Activity.StartActivity("hub.submit_instructions");
        span?.SetTag("instructions.count", instructions.Length);
